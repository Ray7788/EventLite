...
public class Security extends WebSecurityConfigurerAdapter {
	...
	private static final RequestMatcher[] NO_AUTH = { new AntPathRequestMatcher("/webjars/**", "GET"),
			new AntPathRequestMatcher("/**", "GET"), new AntPathRequestMatcher("/h2-console/**"), new AntPathRequestMatcher("/events/mastPost/**")};
	...
	public UserDetailsService userDetailsService() {
		...
		UserDetails mohammad = User.withUsername("Mohammad").password(encoder.encode("Altaisan")).roles(ORGANIZER_ROLE).build();
		return new InMemoryUserDetailsManager(rob, caroline, markel, mustafa, tom, mohammad);
	}
}
