...
public class VenuesControllerApiIntegrationTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void getNextThree() {		
		...
		.contentType(MediaType.APPLICATION_JSON).expectBody().jsonPath("$._embedded.events.length()")
		.value(equalTo(3)).jsonPath("$._embedded.events[0].name").value(containsString("COMP23412 Showcase 01"))
	...
	public void getVenue() {
		...
		.jsonPath("$.name").value(containsString("Alan Turing Building"));
	}
	@Test
	public void getEvents() {
		client.get().uri("/venues/1/events").accept(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk().expectHeader()
		.contentType(MediaType.APPLICATION_JSON).expectBody().jsonPath("$._embedded.events.length()").value(equalTo(3));
	}
}
